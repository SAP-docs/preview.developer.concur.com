...
ESS requires your application to have proper JWT and scopes. Required scopes are:
...
1.	[Authenticate](/api-reference/common/ess/subscription-management.html#ac-jwt)
   * The authentication is done by using a JWT that represents the event subscriber. This can be obtained by calling the [Client Credentials grant](/api-reference/authentication/apidoc.html#client_credentials) type using your OAuth 2.0 credentials. The JWT generated by this grant will have a JWT type of app where your Application ID is the subject of the token.
...
3.	[Create a Subscription](/api-reference/common/ess/subscription-management.html#put-subscription)
   * Confirm your JWT has the scope `events.topic.read` scope.
