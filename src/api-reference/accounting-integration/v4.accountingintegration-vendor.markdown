...
This endpoint allows users to add, update, or delete ERP vendor data that is cached in the SAP Concur system. Upon
...
* **Employee Vendor data has a TTL(Time-To-Live) of 150 days. They need to be re-sent before the expiration so that
  customers' data will stay up to date.**
* Creating Vendors through this API will not automatically import those vendors into the SAP Concur system, it only caches the ERP
  data. Customers will need to add the accounting and employee vendors in the SAP Concur system via the provided UI screens. You
  can use [Invoice Vendor v3 API](api-reference/invoice/v3.1.vendor.html) to import invoice vendors.
