...
`formTypeCode`|`string`|[`FormTypeCode`](#form-type-code)|Form type code to match with the Financial Information document.
`fields`|`array`|[`Field`](#field)|List of fields to match with the Financial Information document.
`connectedLists `|`array`|[`ConnectedList`](#connectedList)|List of connected-list to match with the Financial Information document
...
`invoicePolicies`|`array`|[`InvoicePolicy`](#invoice-policy)|List of Invoice Policies configured in SAP Concur
...
`ledger`|-|`Ledger`| Concur Ledger Information.
...
`purchaseOrderPolicy`|`string`|-|Flag that shows if this is a Purchase Order policy
