...
`paymentMethodType`|`string`|[Payment Status](#schema-payment-method)|The option used to pay the vendor for the good or service. Leave blank to default to the payment method associated with the vendor.
Valid options include the following, which are passed to Payment Manager for processing:
* ACH
* CHECK
The following values are specified on import only if the client has configured the vendor to one these default payment methods:
* CARD
* CLIENT
* VCHER (Voucher)
* WIRE
