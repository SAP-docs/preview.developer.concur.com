...
`State`|`string`|-|The Vendor State. The State fields are now required if the selected vendor has a country defined that requires the state or region to be populated due to the SAP Export Control and Sanctions Compliance Policy. For example, Ukraine is a country that is not completely embargoed so the State field will be required, and the field value validated against the embargoed states. If the selected vendor is from a non-embargoed country, then the State field is not mandatory.
