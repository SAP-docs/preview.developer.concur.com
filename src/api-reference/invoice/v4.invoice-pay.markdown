...
`invoiceDate`|`string`|-|**Optional**: On or After option (date range).
`createDate`|`string`|-|**Optional**: On or After option (date range).
...
`fundingSourceRef`|`string`|-|This identifies the funding source /account used to pay the invoice.(Client Bank Account details)
`cashAccountCode`|`string`|-|
`liabilityAccountCode`|`string`|-|
`fundingRequestRef`|`string`|-|This is the identifier for the debit of the client's account. Used to reconcile to the bank statement.
`checkNumber`|`string`|-|
`fundingSettlementDate`|`string`|-|The date the payment debited the client's bank account.
`fundingCurrency`|`string`|-|Currency in which the payment was funded in by the client to the provider.
`paymentCurrency`|`string`|-|Currency is used to pay the vendors.
