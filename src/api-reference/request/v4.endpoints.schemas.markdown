...
`approved`|`boolean`|`true / false`|Indicates whether this Request is approved.
...
`closed`|`boolean`|`true / false`|Indicates whether this Request is closed.
...
`everSentBack`|`boolean`|`true / false`|Indicates whether the Request has ever been sent back to the employee.
...
`isParentRequest`|`boolean`|`true / false`|Indicates whether this Request is a Budget Request.
...
`pendingApproval`|`boolean`|`true / false`|Indicates whether this Request is pending approval.
...
`isLatest`|`boolean`|`true / false`|If `true`, the comment has been edited since the last workflow transition.
...
`isBlocking`|`boolean`|`true / false`|Defines whether the exception will prevent the Request from being submitted.
...
`allowParticipants`|`boolean`|`true / false`|Whether this policy can add participants (policy is a Budget Request).

## <a name="schema-parentrequestparticipant"></a>Budget Request Participant

Name|Type|Format|Description
---|---|---|---
`participant`|`object`|[`ResourceLink`](#schema-resourcelink)|Participant of the Budget Request.
...
`@deleted`|`boolean`|`true / false`|Whether the Participant should be added or deleted from the Budget Request. If false or not provided then the participant is added.
...
`isPaymentCardRelated`|`boolean`|`true / false`|Indicates if the expected expense entry has the `Is Payment Card Related` field enabled. Value is `true` when the end user has selected this field for this expected expense entry.
...
`percentEdited`|`boolean`|`true / false`|Whether the allocation percent has been edited.
...
`systemAllocation`|`boolean`|`true / false`|Whether the allocation is a system allocation, usually hidden from the user. If displayed to the user, should be read-only.
...
`returnLeg`|`boolean`|`true / false`|Indicates whether this leg is the return leg of a round trip. In case of a `ROUND_TRIP`, if there is no return leg defined, the second segment leg will be considered as the return leg.
...
Name|Type|Format|Description
---|---|---|---
`agencyBooked`|`boolean`|`true / false`|If `true`, this travel is (or has to be) handled by a travel agency.
...
`selfBooked`|`boolean`|`true / false`|If `true`, this travel has been reserved by Concur Travel, or if Concur Travel has retrieved the trip information in the GDS.
...
`booked`|`boolean`|`true / false`|**Required** `true` if this trip is pre-booked by a travel agency. It will pre-select this proposal within the agency proposal comparison screen.
...
`proposal`|`boolean`|`true / false`|**Read only** `true` by default corresponding to the `PROPOSAL` status.
...
`selected`|`boolean`|`true / false`|**Read only** Value becomes `true` when that proposal is selected by the user.
...
`policyCompliant`|`boolean`|`true / false`|Whether the Agency Proposal is policy compliant or not. `true` if proposal is in policy; `false` if proposal is out of policy.
...
`booked`|`boolean`|`true / false`|**Required** `true` if this trip is (or has to be) handled by a travel agency.
`policyCompliant`|`boolean`|`true / false`|Whether the Agency Proposal segment is policy compliant or not. `true` if segment is policy compliant; `false` if segment is out of policy. If no value is provided, the policy compliancy information will not be displayed within the proposal comparison screen.
`comments`|`string`|-|Free text.
...
Name|Type|Format|Description
---|---|---|---
`airConditioning`|`boolean`|`true / false`|Indicates if car has an air conditioner.
