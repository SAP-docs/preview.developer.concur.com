...
`booked`|`boolean`|-|**Required** `true` if this trip is (or has to be) handled by a travel agency.
`policyCompliant`|`boolean`|-|**Read only** Whether the Agency Proposal is policy compliant or not. `true` if policyComplianceLevel is 100.
...
`comments`|`string`|-|Free text.
`startDate`|`date`|[ISO 8601] `yyyy-MM-dd`|The date of the beginning of this segment.
`startTime`|`time`|[ISO 8601] `yyyy-MM-dd`|The time of the beginning of this segment. <br> Required if this field set as mandator for the corresponding segment.
`endDate`|`date`|[ISO 8601] `yyyy-MM-dd`|The date of the ending of this segment.
`endTime`|`time`|[ISO 8601] `yyyy-MM-dd`|The time of the ending of this segment. <br> Required if this field set as mandator for the corresponding segment.
...
`timeZone`|`zoneId`|-|The time zone of the booking. Format: <br>- Fixed offsets: a fully resolved offset from UTC/Greenwich, that uses the same offset for all local date-times; Br>- Geographical regions: an area where a specific set of rules for finding the offset from UTC/Greenwich apply (default group is the IANA Time Zone Database (TZDB)).
...
`classListItemId`|`object`|[`ResourceLink`](#schema-resourcelink)|The unique identifier of the class of service of the air segment (will populate the "Class" list field of the air segment form - if displayed).
`duration`|`integer`|-|**Required** The duration of the booked flight. <br> Must be > 0.
...
`classListItemId`|`object`|[`ResourceLink`](#schema-resourcelink)|The unique identifier of the class of service of the rail segment (will populate the "Class" list field of the rail segment form - if displayed).
...
Name|Type|Format|Description
---|---|---|---
`airConditioning`|`boolean`|-|Indicates if car has an air conditioner. Supported values: `Y` for AC or `N` for No AC.
