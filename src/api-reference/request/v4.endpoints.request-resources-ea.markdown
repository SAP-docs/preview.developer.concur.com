...
`sortField`|`string`|-|The name of the field on which to sort. Supported values: `startDate`, `approvalStatus`, `requestId`. If no sort field value is sent, the default `sortField` `startDate` will be used.
`sortOrder`|`string`|-|Sort order. Supported values: `ASC`, `DESC`. If no sort order value is sent, the default `sortOrder` `DESC` will be used.
`includeDelegateApprovals`|`boolean`|`true` or `false`| If `true` the list of requests returned for the view `TOAPPROVE` will include the requests possible to approve as a delegate approver. If not provided the default value is `false`.
