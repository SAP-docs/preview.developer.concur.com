...
* Connection will always be established using mutual TLS with `webhook.api.concursolutions.com` x509 certificate.
...
* Subscriptions that are not working properly will be deleted after 30 days. This includes not used subscriptions with incomplete setup (missing company ids) and subscriptions with no successful deliveries due to endpoint errors.
* Creating a new subscription please choose a name that would reflect subscription purpose or workflow, eg test, prod, etc. 
