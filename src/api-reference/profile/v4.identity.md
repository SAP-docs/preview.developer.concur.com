...
## Retrieve Users <a name="GET-user-filter"></a>
...
## Create a User's Identity Profile <a name="POST-user-identity"></a>
...
## Update a User's Identity Profile <a name="PATCH-user-identity"></a>
...
## Replace a User's Identity Profile <a name="PUT-user-identity"></a>
...
`notifications`|`boolean`|`true`/`false`|If `true`, notifications have been opted-in for emails.
`type`|`string`|-|A label indicating the attribute's function. Examples: `work`, `home`
`value`|`string`|-|**Required** Email address value.
`verified`|`boolean`|`true`/`false`|If `true`, the email has been verified by the user.
...
`name`|`string`|-|**Required** The emergency contact's name.
...
`relationship`|`string`|-|**Required** The emergency contact's relationship to the user. Supported values: `Spouse`, `Brother`, `Parent`, `Sister`, `Life Partner`, `Other`
`streetAddress`|`string`|-|The full street address component, which may include house number, street name, P.O. box, and multi-line extended street address information.
...
`id`|`string`|-|**Required** Unique identifier for the user, also known as the UUID.
