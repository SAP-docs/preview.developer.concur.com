...
The Identity v4.1 service is designed to create, update, and read user’s core identity profile. This service includes additional search functions that enable clients to filter based on User Identity attributes.
...
## Notes
* `filter` and `startIndex` have been removed from the GET Operation.
* POST .search Operation supports `filter`.
* POST .search Operation supports `cursor` as a replacement to `startIndex`.
...
| `identity.user.sap.read`           | Read a user’s SAP Global ID. <br> Required for integrations using SAP IPS for user management. | GET |
...
### <a name="search-scope-usage"></a> .search Operations
...
* `startIndex` parameter has been removed.
  * To return a maximum number of user profiles, `count` may be used.
  * To paginate a query, `cursor` may be used.
...
## Filter For Users <a name="POST-users"></a>

`/Users/.search` enables clients to filter based on User Identity attributes. Filters allow logical and grouping operators to refine search results. Additional information in [Search Implementation Guide.](/api-guides/profile/identity-search.html)
...
[RFC 7644 Patch Operations](https://www.rfc-editor.org/rfc/rfc7644#section-3.5.2)
