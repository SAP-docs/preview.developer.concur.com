...
In order to enable faster certification, SAP Concur provides the following testing tools that will allow CHS suppliers to validate the majority of functional and non-functional requirements of this API during the development phase. Please use below tools as required both during development and once development is complete and provide the test results to SAP Concur via Smartsheet.
...
Please click on this [link](./v4.tests.zip) to download our functional testing tool package.
   ...
   * Tool also validates basic features of the API, for example search response has properties in specified radius; all properties in details response have hotel images; checkin and checkout dates are valid; all hotel rates have appropriate amenity codes; cancel penalties have valid cancel deadlines; etc.
   * For more detailed instructions on how to run these tests please refer to the README inside the testing tool zip.

### Performance Testing

Please click on this [link](./v4.perf-tests.zip) to download our non-functional testing tool package.
