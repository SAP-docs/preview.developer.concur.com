...
In HSv2, several mechanisms are in place to support vendor provided virtual payments (not managed by SAP Concur) as needed by different suppliers. With HSv4, goal is to streamline support for this vendor provided virtual payment. Below is how this payment option will be supported now.
1. Suppliers wanting to use this feature will need to provide `VENDOR_PROVIDED` as one of the option (or only option if this is a required form of payment) in `acceptedPayments` element of [RoomRate](./v4.endpoints.html#schemaroomrateresponse) as returned in response of [`/hotels/rates`](./v4.endpoints.html#rates-) endpoint. 
