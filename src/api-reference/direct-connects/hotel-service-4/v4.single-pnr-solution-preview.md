...
![](images/diagrams/singlepnr/hotel_cancel_preview.png)
...
When a GDS PNR is modified, Concur detects the new booking and calls the CHS Supplier through [read endpoint](./v4.endpoints.html#read-) to fetch its details. The Supplier looks up the reservation and returns confirmation numbers being the primary target. Concur then augments its internal storage with the returned data so that confirmation numbers can be displayed to users and included in future calls, especially cancellation requests.

Note: In this scenario, Concur will not provide SUPPLIER_CONFIRMATION code in request body but only CONCUR_GDS_REFERENCE and HOTEL_CONFIRMATION ones.

![](images/diagrams/singlepnr/hotel_external_booking.png)