...
Retrieve hotel rate details for rates of a property. This endpoint is used and required when details including `guarantee`, `cancePenalties` and `nightlyRates` breakdown are missing from `/hotels/rates` call or `rateDetailsCallRequired` flag is set to true for a given rate.
  ...
  >Note: The `ratePlanId` returned in the `/ratedetails` response must exactly match the `ratePlanId` originally returned by the `/rates` endpoint. This consistency is
  required to ensure the rate selected during shopping is the same rate being booked at checkout.
...
```json
{
  ...
  "searchSessionToken": "b41168ba-7ee1-4b68-9934-47f5c55337d6",
  "hrefs": {
    "changeNotificationV4": "https://{DATACENTER}.api.concursolutions.com/travel/v4/trips/123e4567-e89b-12d3-a456-426614174002/hotels/change-notification",
    "authV0": "https://{DATACENTER}.api.concursolutions.com/oauth2/v0/token"
  },
  "legalEntity": {
...
```json
{
  ...
  "isModifiable": true,
  "voucherUrl": "https://samplehost.com/samplepath/voucher.pdf",
  "activeSegmentInConcurPNR": true
}
```
...
Modify a specific reservation. For more details about the modification process, check out [Hotel Modify Support for CHS](./v4.hotel-modify-booking.html).
