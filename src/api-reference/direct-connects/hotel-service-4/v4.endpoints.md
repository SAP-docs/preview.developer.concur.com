...
| Name                 | Type                                              | Format | Description                                                                    |
|----------------------|---------------------------------------------------|--------|--------------------------------------------------------------------------------|
| `confirmationCodes`  | [`ConfirmationCode`](#confirmationcode)           | -      | **Required**                                                                   |
...
- `RESERVATION` - maps to OTA Unique ID Type `14` and has no equivalent for HSv2. This will be the record locator of supplier and will be provided in all subsequent read/modify/cancel request on a booking.
- `SUPPLIER_CONFIRMATION` - maps to OTA Unique ID Type `40` and represents supplier's confirmation number. This also be provided in all subsequent read/modify/cancel request on a booking, and it is also the one that is used in the passive segment we create in the GDS. (Equivalent to "14" in HSv2).
- `HOTEL_CONFIRMATION` - maps to OTA Unique ID Type `10`. Used by travelers if they want to change their reservation outside of Concur OBT. If property confirmation code is not available, you may send the aggregator confirmation code in this field. (Equivalent to “1000” in HSv2). This field will appear on the itinerary page (as it does today) together with `SUPPLIER_CONFIRMATION` confirmation number.
- `CANCELLATION` - maps to OTA Unique ID Type `15` and is same as HSv2
- `CONCUR_GDS_REFERENCE` - this is custom code for future use which will be used by Concur to provide Concur Booking Record Locator for passives
