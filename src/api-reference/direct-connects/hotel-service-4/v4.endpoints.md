...
Retrieve the hotel rate details for rates of a property. This endpoint is used and required when details including `guarantee`, `cancePenalties` and `nightlyRates` breakdown are missing from `/hotels/rates` call
...
Please note current definitions of `ConfirmationCodeType` as this differs from what was used in  [Hotel-Service-v2](../hotel-service-2/Introduction.markdown) (HSv2).
- `RESERVATION` - **(Optional)** maps to OTA Unique ID Type `14` and has no equivalent for HSv2. This will be the record locator of supplier and will be provided in all subsequent read/modify/cancel request on a booking. If not provided, this defaults to SUPPLIER_CONFIRMATION code.
- `SUPPLIER_CONFIRMATION` - **(Required)** maps to OTA Unique ID Type `40` and represents supplier's confirmation number. This also be provided in all subsequent read/modify/cancel request on a booking, and it is also the one that is used in the passive segment we create in the GDS. (Equivalent to "14" in HSv2).
- `HOTEL_CONFIRMATION` - **(Optional)** maps to OTA Unique ID Type `10`. Used by travelers if they want to change their reservation outside of Concur OBT. If property confirmation code is not available, supplier may send the aggregator confirmation code in this field. (Equivalent to “1000” in HSv2). This field will appear on the itinerary page (as it does today) together with `SUPPLIER_CONFIRMATION` confirmation number.
...
- `CONCUR_GDS_REFERENCE` - this is custom code for future use (Type `14`) which will be used by Concur to provide Concur Booking Record Locator for passives
