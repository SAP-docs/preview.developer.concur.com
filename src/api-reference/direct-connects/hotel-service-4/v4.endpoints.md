...
* `RESERVATION`: Maps to OTA Unique ID Type `14` and has no equivalent for HSv2. This will be the record locator of supplier and will be provided in all subsequent read/modify/cancel request on a booking. If not provided, this defaults to `SUPPLIER_CONFIRMATION` code.
...
* `CONCUR_GDS_REFERENCE`: This is custom code for future use (Type `14`) which will be used by SAP Concur services to provide Concur Booking Record Locator for passives.
