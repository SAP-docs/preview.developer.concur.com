...
`source`|`object`|-|Details about the source attributed to this rate. **NOTE:** `name` must also be provided along with `logo`, though `logo` if provided is rendered with `name` as title/label. Nothing is displayed as `source` if `suppress` is `true`.|
