...
|Name|Type|Format|Description|
|---|---|---|---|
`cancelDeadline`|`string`|`YYYY-MM-DDThh:mm:ssZ`|**Required** Date string in date-time notation as defined by RFC 3339 with time representing local time at property and corresponding timezone. Timezone can be skipped if not available and just provide date-time in local time. |
