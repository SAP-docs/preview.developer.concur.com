...
`hrefs`|`object`|-||
» `changeNotificationV4`|`string`|`url`|**Required** Callback URL for the Change Notification API. The base URI varies depending on the datacenter where the booking is processed. The {TRIP_ID} should be replaced by the actual Trip ID.|
