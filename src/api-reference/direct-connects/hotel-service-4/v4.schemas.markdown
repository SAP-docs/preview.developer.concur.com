...
`gdsRecord`|[`GDSRecord`](#schemagdsrecord)|-|SAP Concur GDS record details pointing to a PNR to use for this booking, either as an active segment or a passive segment. This will be only populated if using [Single PNR Solution](./v4.single-pnr-solution.html)|
...
## <a id="schemagdsrecord"></a> GDSRecord
...
`pcc`|`string`|-|Psudeo City Code or Office ID (OID) for the GDS account to be used for this booking (active or passive segment).|
`recordLocator`|`string`|-|Record locator of the PNR where this booking should be created (active or passvie)|
