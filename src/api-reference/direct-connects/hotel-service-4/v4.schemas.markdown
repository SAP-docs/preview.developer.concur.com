...
- If `SinglePnrEnabled` exists in Partner Service Vendor > Customize Fields but is not enabled in Company Travel Configuration, it appears in `customFields` with value `"false"`.
- If `SinglePnrEnabled` exists in Partner Service Vendor > Customize Fields and is enabled in Company Travel Configuration, it appears in `customFields` with value `"true"`.
- If `SinglePnrEnabled` is removed from Partner Service Vendor > Customize Fields, it does not appear in `customFields`.
