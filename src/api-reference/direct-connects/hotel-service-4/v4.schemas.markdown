...
`hotelPropertyRefs`|[`HotelPropertyRef`](#schemahotelpropertyref)|-|**Deprecated** - Reference IDs to hotel properties, if available. When provided, `locationSearch` will not be used and may not be set.|
...
**Note: `SinglePnrEnabled` in `customFields`**

The `SinglePnrEnabled` field reflects the Single PNR configuration for the supplier:
- If `SinglePnrEnabled` configuration exists in connector settings on the Travel Config page and is enabled, the value in `customFields` will be `"true"`.
- If `SinglePnrEnabled` configuration exists in connector settings on the Travel Config page but not enabled, the value in `customFields` will be `"false"`.
- If `SinglePnrEnabled` configuration doesn't exist in connector settings on the Travel Config page, it will not appear in `customFields`.

> **Important:** If this is the first time the connector is being configured for a company in Company Travel Configuration, you must save or change the value of the field for it to be reflected in `customFields`.
...
`recommendationReasons`|`Array`|`string`|Optional array of free text strings providing reasoning for why this hotel is recommended. Hotel connectors can highlight up to 3 recommended hotels per search response and provide personalized explanations (e.g., "Based on your traveler history", "Part of your loyalty program", "Preferred by your company"). Maximum of 3 recommended hotels per response.|
...
`giataCode`|`string`|-|GIATA code of hotel catalog if location type searched is `HOTEL`.|
...
|Name|Type|Format|Description|
|---|---|---|---|
`addressLines`|`string`|-|**Required** Street address lines.|
...
`description`|`string`|-|-|
`refundableStatus`|[`RefundableStatus`](#schemarefundablestatus)|-|**Required** Supported values: `UNKNOWN`, `FULLY_REFUNDABLE`, `PARTIALLY_REFUNDABLE`, `NON_REFUNDABLE` (previously `NON-REFUNDABLE` - which will be supported as well).|
`amountPercent`|[`AmountPercent`](#schemaamountpercent)|-||
...
Information about the travel arranger who made the reservation on behalf of the traveler.

|Name|Type|Format|Description|
|---|---|---|---|
`firstname`|`string`|-|**Optional** First name of the travel arranger.|
`lastname`|`string`|-|**Optional** Last name of the travel arranger.|
`contactInfo`|[`ContactInfo`](#schemacontactinfo)|-|**Optional** Contact information.|
...
`arranger`|[`Arranger`](#schemaarranger)|-| Information about the travel arranger who made the reservation on behalf of the traveler.|
...
`hrefs`|`object`|-|Map of reference URLs. Each key represents a specific functionality (e.g., `changeNotificationV4`, `authV0`).The base URI varies depending on the datacenter where the service is running.<br>**Base URI:** `https://{DATACENTER}.api.concursolutions.com`
» `changeNotificationV4`|`string`|`url`|**Required** Callback URL for the Change Notification API. |
» `authV0`|`string`|`url`|**Required** Authentication URL for the booking.|
...
`roomRate`|[`RoomRateDetails`](#schemaroomratedetails)|-|**Required** If status is `CANCELLED`, optional|
`checkin`|`string`|`date`|**Required** Check in date as entered by traveler.|
`checkout`|`string`|`date`|**Required** Check out date as entered by traveler.|
...
## <a id="schemasafetyscore"></a> SafetyScore
...
|Name|Type|Format|Description|
|---|---|---|---|
`bookingId`|`string`|-|SAP Concur system id of an existing booking that needs to be updated, as obtained during a reservation request using the `bookingUuid`, see [`ReservationCriteria`](#schemareservationcriteria).(Required for `oldBookings`)|
...
|Name|Type|Format|Description|
|---|---|---|---|
`RefundableStatus`|`string`|-|Supported values: `UNKNOWN`, `FULLY_REFUNDABLE`, `PARTIALLY_REFUNDABLE`, `NON_REFUNDABLE`|
