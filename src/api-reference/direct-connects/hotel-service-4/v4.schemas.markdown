...
`hrefs`|`object`|-|Map of reference URLs. Each key represents a specific functionality (e.g., `changeNotificationV4`, `authV0`).The base URI varies depending on the datacenter where the service is running.<br>**Base URIs:**<br> `https://{DATACENTER}.api.concursolutions.com` <br> `https://{DATACENTER}.api.concursolutions.com` <br> `https://{DATACENTER}.api.concursolutions.com` <br>`https://{DATACENTER}.api.concursolutions.com`|
» `changeNotificationV4`|`string`|`url`|**Required** Callback URL for the Change Notification API. Ex: `https://{DATACENTER}.api.concursolutions.com/travel/v4/trips/{TRIP_ID}/hotels/change-notification` The {TRIP_ID} should be replaced by the actual Trip ID.|
