...
`gdsRecordLocator`|[`GDSRecordLocator`](#schemagdsrecordlocator)|-|SAP Concur GDS record locator pointing to a PNR to use for this booking, either as an active segment or a passive segment. This will be only populated if using 
`hrefs`|`object`|-|Map of reference URLs. Each key represents a specific functionality (e.g., `changeNotificationV4`, `authV0`).The base URI varies depending on the datacenter where the service is running.<br>**Base URIs:**<br> `https://us2.api.concursolutions.com` <br> `https://eu2.api.concursolutions.com` <br> `https://apj1.api.concursolutions.com` <br>`https://integration.api.concursolutions.com`|
» `changeNotificationV4`|`string`|`url`|**Required** Callback URL for the Change Notification API. Ex: `https://us2.api.concursolutions.com/travel/v4/trips/{TRIP_ID}/hotels/change-notification` The {TRIP_ID} should be replaced by the actual Trip ID.|
» `authV0`|`string`|`url`|**Required** Authentication URL for the booking.|

[Single PNR Solution](./v4.single-pnr-solution.html)|
