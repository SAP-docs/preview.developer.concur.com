...
**Note on `SinglePnrEnabled` in `customFields`:**

The `SinglePnrEnabled` field reflects the Single PNR configuration for the supplier:
- If `SinglePnrEnabled` configuration exists in connector settings in Travel Config page and is enabled there, it appears in `customFields` with value `"true"`.
- If `SinglePnrEnabled` configuration exists in connector settings in Travel Config page, but is not enabled there, it appears in `customFields` with value `"false"`.
- If `SinglePnrEnabled` configuration doesn't exists in connector settings in Travel Config page, it does not appear in `customFields`.
