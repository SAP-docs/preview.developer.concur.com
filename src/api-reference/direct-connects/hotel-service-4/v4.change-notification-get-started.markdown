...
This endpoint allows CHS suppliers to update, cancel/replace or add new hotel booking on existing trip. `tripId` must be already known and any existing bookings can be updated only if supplier is the source for the booking. `tripId` can be obtained from reservation request used for adding hotel booking [`ReservationCriteria`](#./v4.schemas.html#schemareservationcriteria).`tripUuid`
...
---|---|---|---
`tripId`|`string`|-|**Required** ID of user trip/itinerary that contains booking(s) that needs to be updated.`tripId` can be obtained from reservation request used for adding hotel booking [`ReservationCriteria`](#./v4.schemas.html#schemareservationcriteria).`tripUuid`
