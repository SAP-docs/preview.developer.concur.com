...
|Description| Error Id|Value|
|---|---|---|
`List not found`|list.not.found|List not found.|
`Deleted list`|list.deleted|The list is currently deleted.|
`Duplicate item`|item.duplicate.code|This item code is already used by another item in the same list.|
`Parent list item doesn't exist`|item.parent.not.found|Parent `listItem` not found.|
`Parent list item is deleted`|item.parent.deleted|Parent `listItem` has been deleted.|
`Parent list item is in a different list`|item.list.id.not.match.parent|List ID does not match that of parent `listItem`.|
`The child list item cannot be added because it would exceed the maximum allowed list levels (10)`|item.max.level.exceeded|Parent at max level. List items cannot be added to this parent.|
`There exists a list item in the same list category with this list item code. List item code must be unique within a category. If categories are enabled, this list item can be shared between lists.`|item.duplicate.item.shared|The list item already exists in another list with the same list category. This list item already exists in another linked list.|
...
```shell
POST https://us2.api.concursolutions.com/list/v4/lists/530ae443-3b22-a44c-8211-c66d96c58d9e/bulk
...
location: https://us2.api.concursolutions.com/list/v4/lists/530ae443-3b22-a44c-8211-c66d96c58d9e/bulk
...
|Description|Error Id|Value|
|---|---|---|
`List item not found`|item.not.found|`listItem` not found.|
`Parent list item is in a different list`|item.list.id.not.match.parent|List ID does not match that of parent `listItem`.|
`List not found`|list.not.found|List not found.|
`A previously deleted item cannot be updated`|item.deleted|The list item has been deleted.|
`Duplicate item`|item.duplicate.code|This item code is already used by another item in the same list.|
`Duplicate item where the duplicate is deleted`|item.duplicate.code.deleted|This item code is already used by other item that's been deleted.|
...
```shell
PATCH https://us2.api.concursolutions.com/list/v4/lists/530ae443-3b22-a44c-8211-c66d96c58d9e/bulk
Accept: application/json
Accept-Language: en
Content-Type: application/json
Authorization: Bearer {token}
