...
Limitations: 
If the attendee being created matches the duplicate search criteria for another attendee with the same attendee type, an error will be thrown. 
If "Search Across Attendee Types" is enabled, the API will prevent an attendee that has matching duplicate criteria across attendee types that have identical duplicate search fields. 
