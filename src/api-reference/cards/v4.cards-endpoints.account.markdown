...
* The `externalId` is a unique identifier (token) for a card account as defined by an external system (outside SAP Concur). It must not contain the primary account number (PAN).
