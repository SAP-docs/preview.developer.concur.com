...
The Spend Document V4 API provides APIs for ingesting & retrieving receipts and compliance documents. After a successful posting and processing of the receipts, available expenses or receipts are created in Concur Expense. In addition to posting the Receipts, the API also allows for partners to directly post compliance documents on behalf of the user to Concur Expense.

## Prior Versions

Although the Spend Document V4 API is being released for the first time, there is already another Receipts V4 API available that enables the posting of electronic receipts (eReceipts) to Concur. In the future, the Spend Document V4 API will be improved to also support the posting of eReceipts.
...
This endpoint allows you to create a new receipt. The receipt data should be provided in the request body.
...
```bash
curl --location --request POST 'https://us.api.concursolutions.com/spend-documents/v4/receipts' \
...
--header 'Content-Type: application/json' \
-F 'image=@"1.jpg"' \
--F 'receipt="{    "transactionDateTime": "08/08/2022 3:30 PM",
    "amount": {
      "total": "18.47",
      "currency": "GBP",
      "subTotal": "15.05"
