...
|Name|Type|Format|Description|
|---|---|---|---|
|`approvalType`|`string`|`expense`/`payment`/`request`/`purchaseRequest`|Options for changing Expense, Invoice, Request, or Purchase Request delegate options.|
|`approvalHierarchySegments`|`string array`|-|The 1 to 10 hierarchy segment values used to identify the hierarchy node the employee has approval rights to; if blank, resolves to the global group.|
...
|`group`|`string`|-|Group to be associated with the approval rights.|
