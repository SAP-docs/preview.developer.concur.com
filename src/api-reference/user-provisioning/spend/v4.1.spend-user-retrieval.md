...
|`startDate`| `DateTime` | ISO 8601 | The `startDate` field (pattern = "yyyy-MM-dd") specifies the beginning date for the range of results to be included. If not provided, then the standard Java Epoch: 1970-01-01T00:00:00 is considered by default. Currently, this field issued for UAC/DDS historical hydration of users in an entity. |
|`endDate`| `DateTime` | ISO 8601 | The endDate field (pattern = "yyyy-MM-dd") specifies the end date for the range of results to be included. If not provided, then the current date time is considered by default. Currently, this field issued for UAC/DDS historical hydration of users in an entity. |
