...
|Role Code|Role Full Name|Description|Group-Aware|Product Area|
|---|---|---|---|---|
|`SHD_APP_CENTER_ADMIN`|App Center Listing Administrator|The user assigned this role can submit App Center listings. This role is only assigned to certain App Center partners.|N|Y|
...
|`REQ_PROCESSOR_AUDIT`|Request Auditor|**This is one of the Request processor roles.**<br>This is a read-only role. The  client can assign this role to TMCs, to its own internal travel agent(s), or to any other user that needs read-only access to requests.|Y|N|
