...
**Note:** The Approval Feature Hierarchy must be configured within SAP Concur for the approvalGroup attribute to be provisionable.
Hierarchies are a connected list within Concur. The individual approvalGroup node is defined by the list item codes concatenated by a hyphen. For example, if the connected list hierarchy is configured with the structure "Research and Development(R&D) -> Quality Assurance(QA) -> Expense Team(Exp)", the value of the approvalGroup is "R&D-QA-Exp". 
