...
```shell
PATCH https://{datacenterURI}/profile/v4/Bulk/
```
#### Parameters
...
```shell
PUT https://{datacenterURI}/profile/v4/Bulk/
```
#### Parameters
...
```shell
GET https://{datacenterURI}/profile/identity/v4/Users/8f545a90-305f-441b-91cd-a69ad5f548f5
GET https://{datacenterURI}/profile/spend/v4/Users/8f545a90-305f-441b-91cd-a69ad5f548f5
...
```shell
GET https://{datacenterURI}/profile/identity/v4/Users/ef0bffaf-863f-4b3e-9c4c-bd9c9a8b2ea7
```
...
`preferredLanguage`|`string`|-|Indicates the user's preferred written or spoken language. SAP Concur supports these [languages](https://www.concurtraining.com/customers/tech_pubs/SupportedLanguages-client/SupportedLanguages-client.pdf) and [codes](/api-reference/user-provisioning/reference/languageCodes.html). Default: `en-US`
...
**NOTE:** In order to be assigned as an approver, the user must have the corresponding value of spend role for each product. Refer to [Spend Role Extension](#spend-role-extension-schema) for how to assign a spend role. The user being provisoined as an approver must be active within Concur. 
...
|Name|Type|Format|Description|
|---|---|---|---|
|`approver`|[`UserReference`](#user-reference-schema)|-|**Required** when using Spend Approver extension. The `UserReference` of the approver.|
...
**NOTE:** In order to be delegated as a delegate, the user itself must have the same corresponding value of spend role for each product as the user delegated for. Refer to [Spend Role Extension](#spend-role-extension-schema) for how to assign a spend role. The user being provisoined as a delegate must be active within Concur. 
...
|Name|Type|Format|Description|
|---|---|---|---|
|`roleName`|`string`|-|**Required** when using role extension. Spend role for a Spend User.|
|`roleGroups`|`string array`|-|**Required** when using role extension. Group(s) to be associated with the Spend role.|
