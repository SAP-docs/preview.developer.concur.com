...
|Name|Type|Format|Description|
|---|---|---|---|
|`delegateProdCode`|`object`|-|A list of Delegate associated with the Delegate's product code.|
|`canApprove`|`boolean`|`true`/`false`|Determines if delegate can approve.|
|`canPrepare`|`boolean`|`true`/`false`|Determines if delegate can prepare.|
|`canPrepareForApproval`|`boolean`|`true`/`false`|Determines if delegate can prepare for approval.|
|`canReceiveApprovalEmail`|`boolean`|`true`/`false`|Determines if delegate can receive approval emails.|
|`canReceiveEmail`|`boolean`|`true`/`false`|Determines if delegate can receive emails.|
|`canSubmit`|`boolean`|`true`/`false`|Determines if delegate can submit.|
|`canSubmitTravelRequest`|boolean|`true`/`false`|Determines if delegate can submit travel request.|
|`canUseBi`|boolean|`true`/`false`|Determines if delegate can use BI.|
|`canViewReceipt`|`boolean`|`true`/`false`|Determines if delegate can view receipts.|
|`delegate`|`object`|-|**Required** UserReference for the delegate.|
...
|`temporaryDelegationFromDate`|`string`|-|Start date for delegate's temporary approval permissions. Valid ISO 8601 Date|
|`temporaryDelegationToDate`|`string`|-|End date for delegate's temporary approval permissions. Valid ISO 8601 Date|
