...
The User Provisioning Service allows callers to provision and update a user’s profile in the SAP Concur environment. Once a user is provisioned, the users profile is available to support the Identity, Travel and Spend services. The service allows clients and partners to modify profiles in bulk or individually using standard REST verbs to create, update objects following a defined schemas covering a users identity, spend and travel data attritubues. 
The services are supported in both our US and EMEA datacenters. 
...
UPS supports post event notification when provisioning process is complete. Using the Event Subscription service will allows clients and partners to be notified through web services when certain actions take place in connected companies. 

https://developer.concur.com/api-reference/ess/v4.event-subscription.html

Event Type:
* `provisionCompleted`

## <a name="create-users-post"></a>Create a new user with /Users endpoint

Creates user provisioning requests via the /Users endpoint. Recommended for clients who are updating single user profiles at a time. Clients are partners are able to provision identity, spend and travel profile information via this endpoint. 
...
```shell
POST https://us.api.concursolutions.com/provisioning/v4/Users/
```
#### URI
...
Updates to one or more user attritubes. 
Using PATCH is recommended for updating single or multiple data attributes for a single user. PATCH will only update the data attributes listed within the request. 
...
```shell
PATCH https://us.api.concursolutions.com/provisioning/v4/Users/<Concur UUID>
```
...
```shell
PATCH https://us.api.concursolutions.com/provisioning/v4/Users/ef0bffaf-863f-4b3e-9c4c-bd9c9a8b2ea7
```
...
```shell
{
    ...
    "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User": {
        "companyId": "aa076ada-80a9-4f57-8e98-9300b1c3171d",
        "employeeNumber": "John_Updated_56@sap.com"
    }
}
...
Using PUT is recommended for updating all data attributes for a user. 
PUT will replace data attributes for the user listed within the request. 
* When attributes are not provisioned, system default values will be provisioned. 
...
```shell
PUT https://us.api.concursolutions.com/provisioning/v4/Users/<Concur UUID>
```
...
```shell
PUT https://us.api.concursolutions.com/provisioning/v4/Users/ef0bffaf-863f-4b3e-9c4c-bd9c9a8b2ea7
```
```shell
...
Creates one or more new user requests via the /Bulk endpoint. Recommended for clients who are performing bulk updates to multiple users at the same time. 
...
```shell
POST https://us.api.concursolutions.com/provisioning/v4/Bulk/
```
#### URI
...
```shell
PATCH https://us.api.concursolutions.com/provisioning/v4/Bulk/
```
#### URI
...
Replaces one or more existing resources. 
Using PUT is recommended for updating all data attributes for a user. PUT will replace data attributes for the user listed within the request and when attributes are not provisioned, system default values will be provisioned. 
* Limited to 100 operations or record size of 400KB per request.  
...
```shell
PUT https://us.api.concursolutions.com/provisioning/v4/Bulk/
```
#### URI
...
#### Request
```shell
GET https://us.api.concursolutions.com/provisioning/v4/provisions/0e00f6a4-6798-4f66-9f47-1e944b619b2e/status
```
...
Retrieves a detailed provisioning request status. The detailed status returns the success or failure for each extention / service called to create a profile within Concur. The extention responses should be reviewed to determine the provisioing status of each extention. When all extentions return success or no-op, the provisoning is complete. When an error is returned, the error should be reviewed and resolved. 
  
Using ?attributes=operations allows a greater detail from each extension for success or error. 
...
### Request
```shell
GET https://us.api.concursolutions.com/provisioning/v4/provisions/{provision-id}/status?attributes=operations
```
#### Parameters
...
```shell
GET https://us.api.concursolutions.com/provisioning/v4/provisions/0e00f6a4-6798-4f66-9f47-1e944b619b2e/status?attributes=operations
```
...
#### Request
```shell
GET https://us.api.concursolutions.com/provisioning/v4/ResourceTypes
```
...
#### Request
```shell
GET https://us.api.concursolutions.com/provisioning/v4/Schemas
```
