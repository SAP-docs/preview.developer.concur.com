...
|Name|Type|Format|Description|
|---|---|---|---|
|`active`|`boolean`|`true`/`false`|**Required** A Boolean value indicating whether the User is active.|
|`userName`|`string`|-|**Required** The name that can be used to log in to CTE.|
|`addresses`|`object`|-|Physical mailing addresses for the User. Supported values: `work`, `home`, `other`, `billing`, `bank`, `shipping`|
...
|`type`|`string`|-|A label indicating the function of the address. Supported values: `work`, `home`, `other`, `billing`, `bank`, `shipping`|
|`dateOfBirth`|`string`|`YYY-MM-DD`|"The user's date of birth, in YYYY-MM-DD format".|
|`displayName`|`string`|-|The name of the User, suitable for display to end-users.|
|`emails`|`object`|-|**Required** Email addresses for the user. The value SHOULD be canonicalized by the service provider, e.g., 'bjensen@example.com' instead of 'bjensen@EXAMPLE.COM'.|
|`notifications`|`boolean`|`true`/`false`|Notifications opt-in for emails|
|`type`|`string`|-|A label indicating the attribute's function. Supported values: `work`, `home`, `work2`, `other`, `other2`|
|`value`|`string`|-|**Required** Email address value.|
|`verified`|`boolean`|`true`/`false`|Flag to note which email has been verified by the user.|
...
|`postalCode`|`string`|-|The zip code or postal code.|
|`region`|`string`|-|The state or region of the contact.|
...
|`preferenceFirstDayOfWeek`|`string`|-|Preferred first day of the week for the user. Supported values: `Sunday`,`Monday`,`Tuesday`,`Wednesday`,`Thursday`,`Friday`,`Saturday`|
...
|`familyName`|`string`|-|**Required** The family name of the User, or last name in most Western languages (e.g., 'Jensen' given the full name 'Ms. Barbara J Jensen, III').|
...
|`givenName`|`string`|-|**Required** The given name of the User, or first name in most Western languages (e.g., 'Barbara' given the full name 'Ms. Barbara J Jensen, III').|
...
|`phoneNumbers`|`object`|-|Phone number canonicalized to RFC3966 format|
|`issuingCountry`|`string`|-|Issuing country for mobile devices (type=mobile); two-letter code per ISO 3166-1 alpha-2|
|`display`|`string`|-|Phone numbers for the user.|
|`notifications`|`boolean`|`true`/`false`|LNA Opt-in for mobile devices (type=mobile)|
|`primary`|`boolean`|`true`/`false`|If `true`, this is the primary device for mobile devices.|
|`type`|`string`|-|A label indicating the phones's function. Supported Values: `work`, `home`, `mobile`,`fax`,`pager`,`other`,`work2`|
...
|`id`|`integer`|-|valid Rule class id to be assigned|
