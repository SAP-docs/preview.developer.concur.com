...
`identity.user.delete`|Hard delete of users - not recommended. Contact account manager|DELETE
* The DELETE operation should only be used for users that have no transaction history. For users with transaction history, see the [Data Retention policy](https://www.concurtraining.com/customers/tech_pubs/Docs/_Current/SG_Shr/Shr_SG_Data_Retention.pdf).
