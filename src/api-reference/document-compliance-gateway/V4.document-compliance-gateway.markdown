...
| `uuid`| `string` | - | **Required for countries: MX, PL.**<br>Document UUID - field present in compliance document/Government doc unique id.|
| `grossAmount` | `number` | - | **Required.** Sum of amounts before discounts and taxes.|
| `currency` | `string` | ISO 4217 | **Required.** Currency used to express amounts (According to the ISO 4217 codes).|
| `exchangeRate` | `number` | - | Exchange Rate according to currency used.|
...
| `number`| `string` | - | **Required for countries: FR, PL.**<br>Number/Supplier Document Id of the receipt.|
| `formatVersion` | `string` | `enum`: [`3.3`, `4.0`]| Version of the compliance document.|
...
| `paymentType` | `string` | `enum`: [`Cash`, `Credit Card`, `Digital Wallet`] | Means of payment.|
...
|Name|Type|Format|Description|
|---|---|---|---|
| `name` | `string` | - | **Required.** Name of receipt issuer.|
| `country` | `string` | - | Country of receipt issuer.|
| `taxNumber`| `string` | - | **Required for countries: PL, FR.**<br>Taxpayer ID of the vendor.|
