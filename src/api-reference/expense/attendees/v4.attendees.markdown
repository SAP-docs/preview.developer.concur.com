...
|---|---|---|---|
|`attendeeTypeCode`|`string`|`query`|A code that indicates the type of attendee. By entering a value for this parameter, you can limit the results to the attendees who match the specified type.|
...
|`userId`       |`string`|`query`|The unique identifier of the SAP Concur user that owns the attendee. Use [Identity v4 API](/api-reference/profile/v4.identity.html)  to retrieve the `userId`. The user who is performing this API request must have the Web Services Admin (Professional) or Can Administer (Standard) user role to use this parameter.|
|`versionNumber`|`number`|`query` | The attendee's version number. Default value: latest attendee version|
...
|Name|Type|Format|Description|
|---|---|---|---|
|`idList` |`string`|`query` |**Required** A list of comma-separated attendee ID's. Max Value: 10|
|`userId` |`string`|`query`|The unique identifier of the SAP Concur user that owns the attendee. Use [Identity v4 API](/api-reference/profile/v4.identity.html)  to retrieve the `userId`. The user who is performing this API request must have the Web Services Admin (Professional) or Can Administer (Standard) user role to use this parameter.|
