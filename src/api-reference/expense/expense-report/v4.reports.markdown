...
Updates the report header in an un-submitted or submitted expense report specified and is restricted to modify 'Business Purpose', 'Comment', 'Custom/Org unit', 'Name' and 'Paper Receipt Received' fields only (only Company JWT Allowed).
