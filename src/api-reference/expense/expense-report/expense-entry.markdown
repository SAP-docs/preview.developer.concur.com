...
`Comment`	|	`string`	|	-	|	A comment that describes the expense entry. Maximum length: 500 characters.  Disclaimer: This field is not supported on POST/PUT operation, and will always be empty on GET, even if the expense does have an associated comment.
