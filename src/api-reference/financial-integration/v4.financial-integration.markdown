...
Only when the docId is specified in the query parameter the ignoreDocumentStatus value is taken into consideration or else ignoreDocumentStatus  even if set to yes is set to the default value of no and will return only documents that have not been acknowledged/confirmed aka in READY status.
