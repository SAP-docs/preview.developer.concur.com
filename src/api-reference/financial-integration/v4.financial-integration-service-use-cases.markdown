...
## Employee Paid Credit Card <a name="uepcc"></a>
...
When the Request is created, the employee group configuration is evaluated to determine if FGI is enabled. During the workflow when the Funds Reservation step is reached, the workflow is paused, and a Request  document payload is made available to the financial system via the FIS API.

#### Example

The Request with one or more Funds Reservation in the Request workflow.
...
After Request has been submitted, during the Funds Reservation step in the workflow, a Request Payload with instruction "obligate" is made available to the financial system via the FIS API.
...
Once the Request Payload with instruction "obligate" is received, the financial system sends acknowledgement back to the SAP Concur system.
...
The Travel Request payload with instruction "obligate" is made available to the financial system via the FIS API.
...
The Financial system attempts to update the funds reservation against the details in the Request Obligation.
...
[**Acknowledgement JSON Example**](json/FI-API-FundsReservation-ACK-26f00d0618594923a9f66cc6473560dd-Example-1.0.2.json)

As there is no change in the details of Request, there will not be any further funds reserved in the Financial system.  
The Financial System will provide a corresponding message to indicate whether the operation was successful or not.

[**Confirmation JSON Example**](json/FI-API-FundsReservation-CNF-8423f1f533cd439eb4d5fbc8528dd8a0-Example-1.0.2.json)

[**Release Funds Reserved on Request**](json/FI-API-FundsReservation-release-44FBE24939BA8148ADBD88B80C721C59-Example-1.0.2.json)

#### Option 1
When a Request is closed, FGI evaluates the status of all linked Reports. 

* When all the linked Reports are paid, release of funds reserved in Request is initiated.
* A Request Payload with instruction "release" is made available to the financial system via the FIS API.

#### Option 2
When a Request is deleted, a Request Payload with instruction "release" is made available to the financial system via the FIS API.
...
When the Request is created, the employee group configuration is evaluated to determine if FGI is enabled. During the workflow when the Funds Check step is reached, the workflow is paused, and a Request document with instruction "fundsCheck" is made available to the financial system via the FIS API.
 
#### Examples

*	Funds Check as a first step in the Request workflow after submit.
* Funds Check after Funds Reservation step in the Request workflow.
 
#### Example 1: Funds Check as a first step in the Request workflow after submit.
...
After Request has been submitted, during the Funds Check Step in the workflow, a Payload with instruction "fundsCheck" is made available to the financial system via the FIS API.
...
Once the Request  Payload with instruction "fundsCheck" is received, the financial system sends acknowledgement back to the SAP Concur system.
...
#### Example 2: Funds Check after Funds Reservation step in the Request workflow.
...
When the request reaches the funds check step following funds reservation,

#### Case 1
Request was modified. The expense entry Laundry is updated with– 1300 USD

The Request payload with instruction "fundsCheck" is made available to the financial system via the FIS API

[Request Funds Reservation JSON Example]
...
Request Funds Check JSON Example
...
The financial system checks for the funds availaiblity against the details in the Request Payload.
will provide a corresponding message to indicate whether the operation was successful or not
...
When an Expense Report originates from a Travel Request and the Request has been enabled for Funds and Grants Integration (FGI), the resulting report will automatically be enabled for FGI. Furthermore, when the Expense Report is not created from the Travel Request, the employee group configuration is evaluated to determine if FGI is enabled. During the workflow when the Funds Reservation step is reached, the workflow is paused, and a Report Payload with instruction "obligate" is made available to the financial system via the FIS API

#### Example

The Report with one or more Funds Reservation in the Report workflow, linked to one or more Request.
...
After Report has been submitted, during the Funds Reservation step in the workflow, a Payload with instruction "obligate" is made available to the financial system via the FIS API.

[**Report Funds Reservation payload JSON Example**](json/FI-API-FundsReservation-ReportID-B33CEEB1CE814808A06A-Example-1.0.0.json)

Once the Report Payload with instruction "obligate" is received, the financial system sends acknowledgement back to the SAP Concur system.

[**Acknowledgement JSON Example**](json/FI-API-FundsReservation-ACK-2ddb1fe8eb4944b2aa58a09e7e34c6de-Example-1.0.0.json)

The Financial system attempts to reserve funds based on the information contained in the Report payload. Subsequently, the financial system will provide a corresponding message to indicate whether the operation was successful or not.

[**Confirmation JSON Example**](json/FI-API-FundsReservation-CNF-7acc5976287c4ab5acf1bc7195a2593e-Example-1.0.0.json)

### Funds Reservation in the Financial System after Manager Approval

During the second Funds Reservation workflow step,
#### Case 1
Report was modified. The expense entry Airfare is updated with– $ 5.

The Expense Report payload with instruction "obligate" is made available to the financial system via the FIS API.
...
Once the Report Obligation Payload with instruction "obligate" is received, the financial system sends acknowledgement back to the SAP Concur system.

[**Acknowledgement JSON Example**](json/FI-API-FundsReservation-ACK-0da40f8164dd45668e4bb22c2badbaaf-Example-1.0.1.json)

The Financial system attempts to reserve funds based on the information contained in the Report payload.
 Subsequently, the financial system will provide a corresponding message to indicate whether the operation was successful or not.

[**Confirmation JSON Example**](json/FI-API-FundsReservation-CNF-1631bb98e78340699aa3b968757105fe-Example-1.0.1.json)

**Release Report Commitment**


#### Option 1
When an Expense Report is deleted, the Funds Reserved against the deleted expense report is released.
*	A Report Payload with instruction "release" is made available to the financial system via the FIS API.
  
#### Option 2

* When a Report is paid or in other words, the employee is reimbursed, The remaining funds reserved(if any) against the Expense Report is released.
* A Report Payload with instruction "release" is made available to the financial system via the FIS API.
  
[**Expense Report Release JSON Example**](json/FI-API-FundsReservation-release-99509FA3DC4F4159B557-Example-1.0.1.json)

* Further, if the linked Request is closed, and this Request is linked to both Reports 1 and 2, both of which have been paid, then the remaining funds reserved(if any) against the Travel Request is also released.
*	A Request Payload with instruction "release" is made available to the financial system via the FIS API.

[**Travel Request Release JSON Example**]

### Funds Check on Report

When an Expense Report originates from a Travel Request and the Request has been enabled for Funds and Grants Integration (FGI), the 
resulting report will automatically be enabled for FGI. Furthermore, When the Expense Report is not created from the Travel Request, the employee group configuration is evaluated to determine if FGI is enabled. 
During the workflow when the Funds Check step is reached, the workflow is paused, and a Report Payload with instruction "fundsCheck” is made available to the financial system via the FIS API.

#### Examples
* The Report with one or more Funds Check in the Report workflow,

### Sample Set Up

*	Funds Check step right after submit.
*	Funds Check step after Manager approval
...
After Report has been submitted, during the Funds Check step in the workflow, a Payload with instruction "fundsCheck" is made available to the financial system via the FIS API.

[**Report Funds Check JSON Example**](json/FI-API-FundsReservation-fundsCheck-5537EA583D4E486D9469-Example-1.0.0.json)


Once the Report Payload with instruction "fundsCheck" is received, the financial system sends acknowledgement back to the SAP Concur system

[**Acknowledgement JSON Example**](json/FI-API-FundsReservation-ACK-e9fbf821f2c14e738fb2aa693a7e2dbc-Example-1.0.0.json)

The Financial system check for the availability of funds based on the information contained in the Report payload. 
Subsequently, the financial system will provide a corresponding message to indicate whether the operation was successful or not.

[**Confirmation JSON Example**](json/FI-API-FundsReservation-CNF-3e0b085fae6841d58dbcaf7803b587ef-Example-1.0.0.json)

####  Funds Check in the Financial System after Manager Approval

During the second Funds Check workflow step,
#### Case 1
Report was modified. The expense entry Airfare is updated with– 5 USD

The Expense Report payload with instruction "fundsCheck" is made available to the financial system via the FIS API.
...
[**Acknowledgement JSON Example**](json/FI-API-FundsReservation-ACK-e79a43d1211c4ff3ba181500f9e18fe3-Example-1.0.0.json)

The financial system checks for the funds availaiblity against the details in the Report Payload and will provide a corresponding message to indicate whether the operation was successful or not

[**Confirmation JSON Example**](json/FI-API-FundsReservation-CNF-a4ba403357e34581b78fe0e29ffa6c77-Example-1.0.0.json)
