...
Travel Request with one expense item: Laundry – USD 4.
...
For the FIS end point URL, refer to [Get Financial Transactions](/api-reference/financial-integration/v4.financial-integration.html#/).

Upon the receipt of the document by the financial system, it sends an acknowledgment back to the SAP Concur system.

[FIS Acknowledgement URL and Payload](/api-reference/financial-integration/v4.financial-integration.html#post-financial-transaction-acknowledgements-).
...
[FIS Confirmation URL and Payload](/api-reference/financial-integration/v4.financial-integration.html#post-financial-transactions-confirmations-).
...
Request was modified. The expense entry Laundry is updated with – 3 USD
...
* When a Request is deleted, a request payload is provided to the financial system through FIS API, containing the instruction "release".
...
Travel Request with one expense item: Laundry – 4 USD.
...
Following the submission of a request, at the Funds Check step of the workflow, a request payload is provided to the financial system through FIS API, containing the instruction "fundsCheck"
...
Request was modified by manager and approved. The expense entry Laundry is updated with – 3 USD.

A request payload is provided to the financial system through FIS API, containing the instruction "fundsCheck"

[**Request Funds Check JSON Example**](json/FI-API-FundsReservation-fundsCheck-0D637CA450A5485E9C79D2E1C42BBBB8-Example-1.0.0.json)
...
Consider a Report that includes one or more Funds Reservation in the Report workflow, linked to one or more Request.
...
FGI enabled Travel Request is created with one entry – Laundry – 4 USD is approved and an Expense Report is created with one expense item: Laundry – 4 USD.
...
Report was modified. The expense entry Laundry is updated with – 3 USD.
...
* When an Expense Report is deleted, the Funds Reserved against the deleted expense report are released. 
   * A Report Payload with instruction "release" is made available to the financial system via the FIS API. 
* When a Report is paid or in other words, the employee is reimbursed, The remaining funds reserved (if any) against the Expense Report are released. 
   * A Report Payload with instruction "release" is made available to the financial system via the FIS API.
...
Expense Report is created with one expense item: Laundry – 4 USD.
...
[**Report Funds Check JSON Example**](json/FI-API-FundsReservation-fundsCheck-07EC70C056074340ADCA-Example-1.0.0.json)
