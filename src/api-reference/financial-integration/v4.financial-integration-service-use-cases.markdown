...
### Release Request Commitment:
When a Request is closed,  FGI evaluates the status of all linked Reports.
1. 	When all the linked Reports are paid, release of Request commitment is initiated.
2. 	A Request Obligation Payload with instruction "release" is made available to the financial system via the FIS API.
