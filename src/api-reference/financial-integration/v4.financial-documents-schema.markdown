...
`entrySupplierTaxID`|`String`| Alphanumeric. Maximum length 64 characters.| Unique identifier assigned to the Merchant/Supplier by the tax authority for purposes of collecting and reporting taxes. This field applies to Mexico CFDI feature only. For other digital tax compliance countries see "merchantTaxId".
`entryUuid`|`String`| Maximum length 32 characters| Report Entry XML Receipt UUID. Unique identifier assigned by the supplier/merchant to the invoice. This field applies to Mexico CFDI feature only. For other digital tax compliance countries see "supplierInvoiceId".
...
`merchantTaxId`|`String`|Alphanumeric. Maximum length 64 characters.|Unique identifier assigned to the Merchant/Supplier by the tax authority for purposes of collecting and reporting taxes. Merchant Tax ID is populated by the following (in order of precedence): Electronic Receipt v4 with tax data, Company card transaction, and ExpenseIt OCR transaction. For Mexico CFDI see "entrySupplierTaxID".
`supplierInvoiceId`|`String`|Alphanumeric. Maximum length 64 characters.|Unique identifier assigned by the supplier/merchant to the invoice. Used for tax compliance reconciliation between tax authority and customer ERP. This field applies to digital tax compliance countries except Mexico. For Mexico CFDI see "entryUuid".
`govInvoiceId`|`String`|Alphanumeric. Maximum length 64 characters.|Unique identifier assigned by the tax authority in certain jurisdictions. Used for tax compliance reconciliation between tax authority and customer ERP. This value populated in payload only for digital compliance countries where it is available/required.
`companyTaxId`|`String`|-|The official tax Goods and Services Tax Identification Number (GSTIN) of the company/legal entity responsible for the expense, used for tax calculation and compliance purposes.
`hsnSacCode`|`String`|-|A code that identifies the type of goods or services for tax calculation and compliance purposes. HSN applies to goods. SAC applies to services.
`placeOfSupplyState`|`String`|-|Identifies the state or territory where a good or service is considered to be supplied for GST purposes. Used to determine the applicable GST rate and type (IGST, CGST, SGST).
...
| `hasMobileCanteenOrHeavyLoadAttached` | `Boolean`  |`true/false` |Indicates whether a mobile canteen or heavy load is attached.|
| `hasCaravanAttached` | `Boolean`  | `true/false` | Indicates whether a caravan is attached.|
| `hasDogIncluded` | `Boolean`  | `true/false`  | Indicates whether a dog is included.|
| `hasForestOrConstructionSiteRoadInRoute` | `Boolean`  | `true/false` | Indicates whether there are forest or construction site roads on the route.|
| `hasForestRoadInRoute` | `Boolean`  | `true/false`  | Indicates whether there is a forest road in the route.|
| `isMarkedAsHigherRate` | `Boolean`  | `true/false`  | Indicates that the mileage amount is set as the higher rate.|
| `hasMachinery` | `Boolean`  | `true/false`  | Indicates whether machinery is included.|
| `hasTrailerAttached` | `Boolean`  | `true/false` | Indicates whether a trailer is attached.|
