...
The `reportCreated` event is intended to be published when a new expense report has been created. Based on business needs, the event could trigger an application to make API calls to SAP Concur services or start a process in a third-party application.
