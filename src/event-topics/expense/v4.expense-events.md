...
Name|Type|Format|Description
---|---|---|---
`companyId`|`UUID`| RFC 4122| **Required** Company unique identifier from profile service.  `userId`| `UUID`   | RFC 4122| Unique identifier for the User acting on the report resource. This is the user assigned to take action, and does not reflect the delegate who might have executed the action. This value can be NULL when a system instead of a user acts on the report.    
