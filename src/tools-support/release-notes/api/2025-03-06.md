...
[Concur Expense Events](https://developer.concur.com/event-topics/expense/v4.expense-events.html) will be amended to include a `reportCreated` event.  This will be on the same `public.concur.expense.report` topic associated with this event group.  This event will be published upon any report creation whether through the product UI, [Create a New Report API](https://developer.concur.com/api-reference/expense/expense-report/v4.reports.html#create-a-new-report), or other automated sources, for example, Expense Assistant.
