...
### Now Available: Detokenizer v5 – Get Banking Info API

The Detokenizer v5 API now includes a new endpoint to retrieve bank account details:

`POST /detokenizer/v5/bankaccounts/{loginId}`

This endpoint returns the most recently modified active bank account for the given employee `loginId`, with sensitive fields decrypted and the full payload re-encrypted with the caller's AES symmetric key. Authentication requires a Company JWT with the `bankaccount.details.read` scope. This API is available in US2, EU2, APJ1, and CCPS environments.

The `/v5/publickey` endpoint must be called first — it now accepts both `creditcardaccount.read` and `bankaccount.details.read` scopes.

For full details, see [Detokenizer v5](/api-reference/detokenizer/v5.detokenizer-get-started.html).
