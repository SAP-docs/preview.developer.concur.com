...
layout: reference
---
# API Release Notes, November 2025

## New This Month

### Preview: Patch Expense Taxes by Expense ID

The Expense Report Service API suite will be updated to include a mechanism to update tax details for a specified expense entry. These details will include individual tax amounts, tax and reclaim code and custom data. Currently, there is no way to update the tax amounts for an expense entry. This endpoint will initially only support Company JWT authentication. The endpoint requires partners to have a specific scope to update the tax values.

### Now Available: New Client SSL Certificate for ESS webhook.api.concursolutions.com
...
### Now Available: Public Certificate Root Change
