...
The first DTK v5 API will return the Encrypted symmetric RSA key and version of RSA public key that needs to be passed to the second DTK v5 API, along with the mandatory credit card GUID received from client request to provide encrypted credit card account number.
