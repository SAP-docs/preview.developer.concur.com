...
The FIPS Compliant v5 Credit Card Detokenization API is available for all environments. This compliance-driven initiative aligns with the Federal Information Processor Standards (FIPS) to ensure robust protection of sensitive data, per U.S. federal requirements. For customers with entities in these environments with individually billed, company paid (IBCP) card programs, the new Detokenizer (DTK) functionality will be integral to their payment allocation process when reimbursing their corporate card programs outside of SAP Concur.
