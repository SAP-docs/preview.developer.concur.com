...
### Now Available: Travel Allowance Service from Third-Party Solutions 

The Travel Allowance v4 APIs also support writing travel allowance itinerary data into the SAP Concur platform from a third-party solution. Customers and Partners can use the APIs to build custom solutions to enhance their business processes wherever travel allowance itineraries need to be created and updated from a third-party solution. This could be solutions for time recording, service management, staffing solutions in CRM systems, booking tools, and many more. The new feature is automatically available, so no additional configuration or activation steps are required.

### Now Available: New Languages Supported 

The Malay and Slovenian languages have been added to the supported locales. For information about Locale Codes please see the topic on the [SAP Help Portal](https://help.sap.com/docs/SAP_CONCUR/b024dbc03488481b9ed0ae8f3dfcaafb/1b8327386caf101489a99a72941e2ee5.html?version=2025_03).

### Preview: Expense v4 DELETE APIs – Reports v4, Expenses v4

The Expense Report Service API suite will be updated to include APIs to delete unsubmitted reports and expenses.

### Preview: Deprecation of Attendees v1, v1.1, and v2

The Attendees v1, v1.1, and v2 were deprecated as of October 9, 2018.
...
For detailed information, check out [Single PNR Solution for CHS](/api-reference/direct-connects/hotel-service-4/v4.single-pnr-solution.html).

### Preview: New Report Events for Concur Expense 

[Concur Expense Events](/event-topics/expense/v4.expense-events.html) will be amended to include two new events: `reportUpdated` and `reportDeleted`. They will be on the same `public.concur.expense.report` topic associated with this event group. These events will be published upon any report `update` or `delete` action whether through the product UI, an API such as the [Update Unsubmitted Report](/api-reference/expense/expense-report/v4.reports.html#update-unsubmitted-report-), or other automated sources such as the Expense Assistant. 
