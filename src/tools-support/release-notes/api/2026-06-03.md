...
### Preview: Preview: Trips v5 API with Configurable Trip Event Subscriptions

The Trips v5 API will provide customers and partners with a scalable and performant way to retrieve trip data based on a configurable event-based subscription model. The API will support the divisional view construct for TMC partners, however the event subscriptions would need the filter configuration setup to the appropriate travel configurations. Partners will receive only the trip and booking details relevant to their use cases, giving them a cleaner, more focused data set that simplifies their integration and reduces unnecessary noise.
