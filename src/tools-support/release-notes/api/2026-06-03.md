---
title: SAP Concur Developer Center - API Release Notes, June 2026
layout: reference
---
# API Release Notes, June 2026
...
* Meeting Management— Create, retrieve, update, and soft-delete meetings

* Attendee Management — Add individual or bulk attendees, including non-employee profiles auto-created as sponsored guests of "Meeting Traveler" type

* Non-Employee Attendee Authentication — Generate OTP-based login URLs enabling sponsored guests to authenticate directly into the Concur booking flow

* Travel Discount Management — Attach negotiated airline discounts to a meeting

* Meeting Match — Configure a date window to automatically associate attendee travel bookings to the correct meeting

* Audit History — Retrieve a paginated audit trail of all changes to a meeting and its entities

* Third-Party Integration — Partners will be able to link meetings to their own systems via `partnerMeetingId`
...
Effective June 8, 2026, the Company Cards Transactions v1 API will be deprecated. This has been replaced by Cards v4 API. Decommission will follow.

### Preview: Trips v5 API with Configurable Trip Event Subscriptions

The Trips v5 API will provide customers and partners with a scalable and performant way to retrieve trip data based on a configurable event-based subscription model. This API will support the divisional view construct for TMC partners, however the event subscriptions would need the filter configuration setup to the appropriate travel configurations. Partners will receive only the trip and booking details relevant to their use cases, giving them a cleaner, more focused data set that simplifies their integration and reduces unnecessary noise.

It will be available in the US2, EU2, and APJ1 data centers.

**Key capabilities:**

* GET Trip by ID (`GET /travel/v5/trips/{id}`) — Returns a trip record including all booking verticals (Air, Hotel, Car, Rail, Ground/Extras).

* Data Minimization via Named Scopes — Partners are assigned one or more scopes (`travel.trip.read`, `travel.trip.{namedschema}.read`) and only receive the data their scopes permit.

* Configurable Trip Event Subscriptions — Company Admins can configure appropriate trip event subscription filter criteria, for partners to receive desired trip events. A new Admin UI lets Company Admins view and update all connected partners' subscription criteria.

* Divisional View Support for TMC Partners — TMC partners only access trips belonging to the travel configurations they manage for a given company by configuring filter criteria for their partner apps to the relevant travel configurations.
...
Two fields will be removed from Access Tokens (JWTs) and ID Tokens: `concur.profile` and `concur.app`. These fields contain fully formed URLs pointing to Profile v1 resources, for example:
...
Date|API|Preview
---|---|---
[05/2026](/tools-support/release-notes/api/2026-05-04.html)|Legal Entity v4.1 API|Will allow customers and partners to manage legal entity profiles within their company structure using SCIM standards. This API will support creating, retrieving, updating, replacing, and searching legal entities via Bearer token authentication.
