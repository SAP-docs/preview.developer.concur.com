...
* Data Minimization via Named Scopes — Assigns one or more scopes (`travel.trip.read`, `travel.trip.{namedschema}.read`) to partners, who only receive the data their scopes permit.

* Configurable Trip Event Subscriptions — Allows Company Admins to configure trip event subscription filter criteria for partners to receive desired trip events. A new Admin UI will let Company Admins view and update all connected partners' subscription criteria.

* Divisional View Support for TMC Partners — Restricts TMC partners to trips belonging to the travel configurations they manage for a given company, configured via filter criteria for their partner apps.
