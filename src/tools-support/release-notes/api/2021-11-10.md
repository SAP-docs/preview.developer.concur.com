...
The [Company Request Token self-service tool](/api-reference/authentication/company-auth.html) enables clients to generate the Company Request Token that is required to request a JSON web token (JWT) when connecting to APIs in the SAP Concur platform.
